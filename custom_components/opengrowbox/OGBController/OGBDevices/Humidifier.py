...
class Humidifier(Device):
    HUMIDIFIER_MODES = ["auto", "eco", "comfort", "baby", "sleep", "home"]
    ...
    def _checkHumidifierModes(self):
        """Check if humidifier entity has mode capabilities."""
    ...
    async def set_humidifier_mode(self, mode):
        """Set humidifier mode."""
        if not self.humidifierEntityId or not self.hass:
            return
    ...
    async def increaseAction(self, data):
        """Turns humidifier on or increases mode."""
        if self.isDimmable:
    ...
    async def reduceAction(self, data):
        """Turns humidifier off or reduces mode."""
