...
from ..data.OGBDataClasses.OGBPublications import (OGBDLIPublication,
                                              OGBPPFDPublication,
                                              OGBWaterPublication)
...
class Sensor:
    ...
    def __init__(
        ...
        self.event_manager.on("CalibrateSensor", self.calibrateSensor)
        self.event_manager.on("SetThresholds", self.setThresholds)

        # NEU: Event für automatische Updates registrieren
        self.event_manager.on("SensorUpdate", self.handleSensorUpdate)
        ...
        for context in ["air", "water", "soil", "light"]:
            ...
            for sensor_type, sensors in context_sensors.items():
                for sensor_config in sensors:
                    ...
                    lines.append(
                        f"║   {alert} [{sensor_type}] {entity_id}: {value_str} {unit}"
                    )

                    # Zeige Schwellwerte falls gesetzt
                    if (
                        sensor_config.get("threshold_min") is not None
                        or sensor_config.get("threshold_max") is not None
                    ):
                        ...
                        lines.append(
                            f"║      Current: [{thresh_min} - {thresh_max}] | Optimal: [{optimal_min} - {optimal_max}]"
                        )
    ...
    def sensorPlatformIdent(self):
        ...
        for entry in self.deviceData:
            ...
            # Include BOTH entity-specific labels AND device-level labels (entity=None)
            entity_labels = [
                lbl for lbl in self.labelMap 
                if lbl.get("entity") == entity_id or lbl.get("entity") is None
            ]
        ...
        # Plattform setzen
        self.devicePlatform = (
            platform_set.pop() if len(platform_set) == 1 else list(platform_set)
        )
        self.sensorMap = {
            "sensors": sensor_map,
            ...
            for context in ["air", "water", "soil", "light"]:
                ...
                for sensor_type, sensor_entries in context_sensors.items():
                    for sensor_entry in sensor_entries:
                        await self._initializeSensorType(
                            sensor_type, sensor_entry, context
                        )
    ...
    async def _initializeSensorType(self, sensor_type, sensor_entry, context):
        ...
        # CRITICAL: Include medium_label and room for MediumSensorUpdate events
        # These are needed by OGBMediumManager to route updates to correct medium
        sensor_config = {
            "entity_id": entity_id,
        ...
        # MEMORY FIX: Use list but prevent duplicates by checking entity_id first
        if sensor_type not in self.sensorReadings[context]:
            self.sensorReadings[context][sensor_type] = []

        # Check for duplicate and update if exists, otherwise append
        existing_idx = None
        for idx, existing in enumerate(self.sensorReadings[context][sensor_type]):
        ...
        if existing_idx is not None:
            ...
            self.sensorReadings[context][sensor_type][existing_idx] = sensor_config
        else:
        ...
        effective_medium_label = entity_medium_label or self.medium_label
        
        # Update sensor_config with the effective medium_label for later use
        if effective_medium_label:
            sensor_config["medium_label"] = effective_medium_label
        ...
        # Emit SensorInitialized event for FallBackManager monitoring
    ...
    async def _register_sensor_to_medium(self, entity_id: str, sensor_type: str, medium_label: str = None):
        """Registriert diesen Sensor bei seinem Medium.
        
        Args:
            entity_id: The entity ID of the sensor
            sensor_type: The type of sensor (temperature, moisture, etc.)
            medium_label: The medium label to register to (uses self.medium_label if not provided)
        """
        try:
            # Use provided medium_label or fall back to device-level
            effective_label = medium_label or self.medium_label
            if not effective_label:
                _LOGGER.warning(f"[{self.room}] No medium_label for registration of {entity_id}")
                return
            ...
            if not sensor_config:
                _LOGGER.error(f"[{self.room}] NO Sensor Config {entity_id} found for medium registration")
                return

            # Aktuellen Wert und zusätzliche Infos holen
            current_value = sensor_config.get("state") or sensor_config.get(
                "last_reading"
            )
            context = sensor_config.get("context", "unknown")
            
            _LOGGER.debug(
                f"[{self.room}] RegisterSensorToMedium: {entity_id} -> {effective_label}"
            )
        ...
        try:
            ...
            if new_state is not None and new_state not in ["unavailable", "unknown"]:
                ...
                try:
                    ...
                    # Emit MediumSensorUpdate for sensors that are registered to a medium
                    # soil context = substrate sensors, medium_label = explicitly tagged sensors
                    # water context is for HYDRO tank sensors, NOT medium sensors
                    if sensor_config["context"] == "soil" or self.medium_label:
                        _LOGGER.debug(
                            f"[{self.room}] MediumSensorUpdate: {entity_id} = {numeric_value}"
                        )
                        await self.event_manager.emit(
                            "MediumSensorUpdate", sensor_config
                        )
                    ...
                    if sensor_config["sensor_type"] == "light":
                        if "_lumen" in entity_id:
                            ...
                            unit = "lux"
                        growSpace = self.data_store.get("growAreaM2")
                        lightStart = self.data_store.getDeep("isPlantDay.lightOnTime")
                        lightStop = self.data_store.getDeep("isPlantDay.lightOffTime")
                        lightDuration = hours_between(lightStart, lightStop)

                        # Get LED type and factor from dataStore
                        led_type = (
                            self.data_store.getDeep("Light.ledType")
                            or "fullspektrum_grow"
                        )
                        lux_factor = (
                            self.data_store.getDeep("Light.luxToPPFDFactor") or 15.0
                        )
                        ...
                        self.data_store.setDeep("tentData.DLI", dli)
                        self.data_store.setDeep("tentData.PPFD", ppfd)

                        await _update_specific_sensor(
                            "ogb_ppfd_", self.room, ppfd, self.hass
                        )
                        await _update_specific_sensor(
                            "ogb_dli_", self.room, dli, self.hass
                        )

                        self.data_store.setDeep("Light.DLICurrent", dli)
                        self.data_store.setDeep("Light.PPFDCurrent", ppfd)

                        DLIPub = OGBDLIPublication(Name="DLIUpdate", DLI=dli)
                        await self.event_manager.emit("DLIUpdate", DLIPub)
                        PPFDPub = OGBPPFDPublication(Name="PPFDUpdate", PPFD=ppfd)
                        await self.event_manager.emit("PPFDUpdate", PPFDPub)
                        return

                    if sensor_config["context"] == "water":
                        updated = False
                        if "_ec" in entity_id:
                            self.data_store.setDeep("Hydro.ec_current", numeric_value)
                            updated = True
                        elif "_tds" in entity_id:
                            self.data_store.setDeep("Hydro.tds_current", numeric_value)
                            updated = True
                        ...
                        if updated:
                            ...
                            tds_current = self.data_store.getDeep("Hydro.tds_current")
                            ph_current = self.data_store.getDeep("Hydro.ph_current")
                            oxi_current = self.data_store.getDeep("Hydro.oxi_current")
                            sal_current = self.data_store.getDeep("Hydro.sal_current")
            ...
            _LOGGER.error(
                f"Fehler beim Verarbeiten von SensorUpdate für {entity_id}: {e}",
                exc_info=True,
            )
            ...
            if sensor_type in self.sensorReadings[ctx]:
                for sensor_config in self.sensorReadings[ctx][sensor_type]:
                    ...
                    reading = {
                        ...
                        "timestamp": (
                            sensor_config.get("last_update").isoformat()
                            if sensor_config.get("last_update")
                            else None
                        ),
    ...
    async def _readSingleSensor(self, sensor_config):
        ...
        try:
            ...
            _LOGGER.error(
                f"Fehler beim Lesen von Sensor {sensor_config['entity_id']}: {e}"
            )
            return None
    ...
    def _checkSingleSensor(self, sensor_config):
        ...
        if last_update:
            ...
            if age > timedelta(minutes=5):
                _LOGGER.warning(
                    f"Sensor {entity_id}: Letzte Lesung ist {age.seconds}s alt"
                )
                return False
        else:
